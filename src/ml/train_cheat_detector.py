...
class CheatDetectorTrainer:
    ...
    def extract_features_from_game(self, pgn_game, is_cheater=False):
        ...
        for node in pgn_game.mainline():
            ...
            features = self.extract_position_features(board, move_accuracy, eval_score, move_num, is_cheater)
    ...
    def extract_position_features(self, board, move_accuracy, eval_score, move_num, is_cheater):
        ...
        features.append(eval_score / 100.0)
        features.append(move_num / 40.0)
        legal_moves = len(list(board.legal_moves))
        features.append(legal_moves / 50.0)
        white_material = sum([
    ...
    def create_synthetic_data(self):
        ...
        X_legit[:, 0] = np.random.beta(2, 5, n_samples // 2)
        X_legit[:, 8] = np.random.beta(2, 5, n_samples // 2)
        X_legit[:, 9] = np.random.uniform(0.1, 0.3, n_samples // 2)
        y_legit = np.zeros(n_samples // 2)
        X_cheat = np.random.randn(n_samples // 2, n_features)
        X_cheat[:, 0] = np.random.beta(9, 2, n_samples // 2)
        X_cheat[:, 8] = np.random.beta(9, 2, n_samples // 2)
        X_cheat[:, 9] = np.random.uniform(0, 0.1, n_samples // 2)
        X_cheat[:, 11] = np.random.beta(9, 2, n_samples // 2)
...
def main():
    ...
    parser.add_argument('--data-dir', type=str, default='data/games', help='Directory containing PGN files')
    parser.add_argument('--model-path', type=str, default='models/cheat_detector_v2.h5', help='Path to save trained model')
