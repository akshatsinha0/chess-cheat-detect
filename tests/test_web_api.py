...
class TestWebAPI:
    ...
    def test_make_move_valid(self, mock_analyze, client):
        mock_analyze.return_value = {'best_move': 'e2e4','evaluation': {'value': 30, 'type': 'cp'}}
    ...
    def test_make_move_invalid(self, client):
        response = client.post('/api/make_move', json={'move': 'e2e5'})
    ...
    def test_analyze_fen_invalid(self, client):
        response = client.post('/api/analyze_fen', json={'fen': 'invalid_fen'})
        assert response.status_code == 400
        data = json.loads(response.data)
        assert data['status'] == 'error'
    ...
    def test_import_pgn(self, mock_analyze, mock_read_game, client):
        mock_game = MagicMock()
        mock_game.headers = {'White': 'Player1','Black': 'Player2','Result': '1-0','Date': '2023.01.01'}
